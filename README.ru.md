...
[![Бинарник](https://img.shields.io/badge/%D0%B1%D0%B8%D0%BD%D0%B0%D1%80%D0%BD%D0%B8%D0%BA-6.3%20%D0%9C%D0%91%20%D1%81%D1%82%D0%B0%D1%82%D0%B8%D1%87%D0%B5%D1%81%D0%BA%D0%B8%D0%B9-8a8a8a?style=flat-square)](#сборка-из-исходников)
...
Один статический бинарник. Без OpenRGB, без служб, без среды выполнения.
...
- **Ни одной зависимости.** `ldd` отвечает *не является динамическим
  исполняемым файлом*. Работает на любом x86-64 Linux.
- **Одна программа.** Клавиатура открывается напрямую через `/dev/hidraw`,
  посредника, который можно случайно закрыть, нет.
- **Подсветка остаётся после выхода.** Устройство держит последний кадр, пока
  есть питание.
- **12 эффектов** с живым предпросмотром того самого кадра, что уходит в порт.
...
| **Железо** | HyperX Alloy Origins (`03f0:0591`), полноразмерная |
| **Среда выполнения** | не нужна — бинарник статический |
| **Сборка** | Go 1.21+, только если собираете сами |

Для окна используется браузер на основе Chromium в режиме приложения. Если есть
только Firefox, интерфейс откроется обычным окном.
...
| **Звёзды** | **Дождь** | **Огонь** |
| **Змейка** | **Волны от нажатий** | **Вспышка по нажатию** |

Последние два реактивны и накладываются поверх любого другого эффекта со своими
скоростью, затуханием и цветом.
